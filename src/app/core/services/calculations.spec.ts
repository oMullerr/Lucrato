...
describe('groupByProduct', () => {
  ...
  /** Computa lotes e vendas com o mesmo pipeline puro da app. */
  function compute(purchases: Purchase[], sales: Sale[], settings = makeSettings()) {
    const cp = purchases.map(p => calculatePurchase(p, sales, settings));
    const cs = sales.map(s => calculateSale(s, purchases));
    return { cp, cs };
  }

  it('agrupa lotes do mesmo produto e calcula custo médio PONDERADO', () => {
    const { cp, cs } = compute(
      [
        makePurchase({ id: 'C001', product: 'Pipoqueira', quantityPurchased: 10, unitCost: 100, purchaseShipping: 50, otherCosts: 0, receiptDate: '2026-02-25' }),
        makePurchase({ id: 'C002', product: 'Pipoqueira', quantityPurchased: 5, unitCost: 120, purchaseShipping: 0, otherCosts: 0, receiptDate: '2026-02-25' }),
      ],
      [],
    );
    const groups = groupByProduct(cp, cs);

    expect(groups).toHaveLength(1);
    const g = groups[0];
    expect(g.product).toBe('Pipoqueira');
    expect(g.lotCount).toBe(2);
    expect(g.totalPurchased).toBe(15);
    expect(g.totalInvested).toBe(1650); // 1050 + 600
    // Ponderado: 1650 / 15 = 110 (NÃO a média simples de 105 e 120 = 112,5)
    expect(g.avgUnitCost).toBe(110);
    expect(g.currentStock).toBe(15);
    expect(g.idleCapital).toBe(1650);
  });

  it('agrupa por nome TRIMADO (" Café " e "Café" viram um só grupo)', () => {
    const { cp, cs } = compute(
      [
        makePurchase({ id: 'C001', product: ' Café ', receiptDate: '2026-02-25' }),
        makePurchase({ id: 'C002', product: 'Café', receiptDate: '2026-02-25' }),
      ],
      [],
    );
    const groups = groupByProduct(cp, cs);
    ...
    expect(groups).toHaveLength(1);
    expect(groups[0].product).toBe('Café');
    expect(groups[0].lotCount).toBe(2);
  });

  it('consolida vendas concluídas: lucro total, margem média e lucro médio por unidade', () => {
    const { cp, cs } = compute(
      [makePurchase({ id: 'C001', product: 'X', quantityPurchased: 10, unitCost: 50, purchaseShipping: 0, otherCosts: 0, receiptDate: '2026-02-25' })],
      [
        makeSale({ id: 'V001', batchId: 'C001', quantitySold: 2, unitPrice: 100, feePercentage: 0.1, shippingType: 'correios', sellerShipping: 10, discount: 0, otherCosts: 0, status: 'Concluída' }),
        makeSale({ id: 'V002', batchId: 'C001', quantitySold: 1, unitPrice: 100, feePercentage: 0.1, shippingType: 'correios', sellerShipping: 10, discount: 0, otherCosts: 0, status: 'Concluída' }),
      ],
    );
    const g = groupByProduct(cp, cs)[0];

    // V001: net 170, custo 100 → lucro 70 | V002: net 80, custo 50 → lucro 30
    expect(g.totalSold).toBe(3);
    expect(g.currentStock).toBe(7);
    expect(g.totalRevenue).toBe(300);       // 200 + 100 (gross)
    expect(g.totalNetProfit).toBe(100);     // 70 + 30
    expect(g.avgNetMargin).toBeCloseTo(100 / 300, 10);
    expect(g.avgProfitPerUnit).toBeCloseTo(100 / 3, 10);
  });

  it('produto sem vendas: lucro 0 e médias undefined', () => {
    const { cp, cs } = compute(
      [makePurchase({ id: 'C001', product: 'X', receiptDate: '2026-02-25' })],
      [],
    );
    const g = groupByProduct(cp, cs)[0];

    expect(g.totalSold).toBe(0);
    expect(g.totalNetProfit).toBe(0);
    expect(g.avgNetMargin).toBeUndefined();
    expect(g.avgProfitPerUnit).toBeUndefined();
  });

  it('ignora vendas não concluídas nos totais', () => {
    const { cp, cs } = compute(
      [makePurchase({ id: 'C001', product: 'X', quantityPurchased: 10, unitCost: 50, receiptDate: '2026-02-25' })],
      [
        makeSale({ id: 'V001', batchId: 'C001', quantitySold: 2, status: 'Concluída' }),
        makeSale({ id: 'V002', batchId: 'C001', quantitySold: 5, status: 'Cancelada' }),
      ],
    );
    const g = groupByProduct(cp, cs)[0];

    expect(g.totalSold).toBe(2);
    expect(g.currentStock).toBe(8);
  });

  it('vincula a venda ao grupo pelo batchId, não pela string denormalizada', () => {
    const { cp, cs } = compute(
      [makePurchase({ id: 'C001', product: 'X', quantityPurchased: 10, unitCost: 50, receiptDate: '2026-02-25' })],
      [makeSale({ id: 'V001', batchId: 'C001', product: 'NOME ERRADO', quantitySold: 2, status: 'Concluída' })],
    );
    const g = groupByProduct(cp, cs)[0];

    expect(g.product).toBe('X');
    expect(g.totalSold).toBe(2);
  });

  it('status do grupo = pior status ativo entre os lotes (Parado vence Em Estoque)', () => {
    const settings = makeSettings({ yellowAlertDays: 30, redAlertDays: 60 });
    const { cp, cs } = compute(
      [
        makePurchase({ id: 'C001', product: 'X', receiptDate: '2025-12-01' }), // ~90 dias → Parado
        makePurchase({ id: 'C002', product: 'X', receiptDate: '2026-02-25' }), // ~4 dias → Em Estoque
      ],
      [],
      settings,
    );
    ...
    expect(g.status).toBe('Parado');
  });

  it('coleta categorias e fornecedores distintos', () => {
    const { cp, cs } = compute(
      [
        makePurchase({ id: 'C001', product: 'X', category: 'A', supplier: 'S1', receiptDate: '2026-02-25' }),
        makePurchase({ id: 'C002', product: 'X', category: 'B', supplier: 'S1', receiptDate: '2026-02-25' }),
      ],
      [],
    );
    const g = groupByProduct(cp, cs)[0];

    expect(g.categories).toEqual(['A', 'B']);
    expect(g.suppliers).toEqual(['S1']);
  });

  it('produtos diferentes geram grupos diferentes', () => {
    const { cp, cs } = compute(
      [
        makePurchase({ id: 'C001', product: 'X', receiptDate: '2026-02-25' }),
        makePurchase({ id: 'C002', product: 'Y', receiptDate: '2026-02-25' }),
      ],
      [],
    );
    const groups = groupByProduct(cp, cs);
    ...
    expect(groups.map(g => g.product).sort()).toEqual(['X', 'Y']);
  });
});
